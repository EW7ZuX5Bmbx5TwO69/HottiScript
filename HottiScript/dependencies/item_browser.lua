...
browser.table_copy = function(obj)
    ...
    for k, v in pairs(obj) do
        res[browser.table_copy(k)] = browser.table_copy(v)
    end
    return res
end

browser.search = function(search_params)
    if search_params.page_size == nil then
        search_params.page_size = max_search_results
    end
    if search_params.page_number == nil then
        search_params.page_number = 0
    end
    if search_params.menus == nil then
        search_params.menus = {}
    end
    if search_params.results == nil then
...
browser.search_items = function(folder, query, results)
    ...
    if #results > max_search_results then
        return results
    end
...
browser.browse_item = function(parent_menu, this_item, add_item_menu_function, browse_params)
    if browse_params == nil then
        browse_params = {}
    end
    if this_item.items ~= nil then
        local menu_list = parent_menu:list(this_item.name .. " (" .. #this_item.items .. ")", {},
